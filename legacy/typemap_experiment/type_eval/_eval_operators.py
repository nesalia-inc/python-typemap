...
def cached_box(cls, *, ctx: EvalContext):
    ...
    # Avoid recursion: skip UpdateClass processing if already inside it
    # This prevents infinite recursion when _create_updated_class evaluates Attrs[T]
    if not getattr(ctx, '_in_update_class', False):
...
def _eval_init_subclass(
    ...
    for abox in box.mro[1:]:  # Skip the type itself
        with _child_context() as ctx:
            # Set flag EARLY to prevent recursion when evaluating __init_subclass__ return type
            # This prevents infinite loop when InitFnType[T] -> Attrs[T] -> cached_box -> _eval_init_subclass
            ctx._in_update_class = True
...
def _create_updated_class(
    ...
    # Set flag to prevent recursion in cached_box
    old_in_update_class = getattr(ctx, '_in_update_class', False)
    ctx._in_update_class = True

    try:
        return _create_updated_class_impl(box, ms, ctx)
    finally:
        ctx._in_update_class = old_in_update_class


def _create_updated_class_impl(
    box: _apply_generic.Boxed, ms: typing.Sequence[Member], ctx: EvalContext
) -> type:
    t = box.cls
    dct: dict[str, object] = {}
    ...
    # Copy existing annotations and default values from the original class
    # This preserves the original attributes when UpdateClass adds new members
    dct["__annotations__"] = annos = {}
    if hasattr(t, '__annotations__'):
        for name, anno in t.__annotations__.items():
            annos[name] = anno
    # Copy default values from class __dict__
    for name in t.__dict__:
        if name.startswith('__'):
            continue
        val = t.__dict__[name]
        if not callable(val) and not isinstance(val, type):
            # Copy class variables with defaults
            dct[name] = val
