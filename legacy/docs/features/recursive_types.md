...
All proposed recursive patterns were tested and work:

```python
from typemap.type_eval import eval_typing
from typemap_extensions import Attrs, Iter, NewProtocol, Member

# Test 1: LinkedList as class
class LinkedList[T]:
    value: T
    next: LinkedList[T] | None = None

eval_typing(LinkedList[int])
# Works! Has: value: int, next: LinkedList[int] | None ✅
...
eval_typing(BinaryTree[str])  # Works! ✅

# Test 3: TreeNode with children
class TreeNode[T]:
    value: T
    children: list[TreeNode[T]] = []

eval_typing(TreeNode[int])  # Works! ✅

# Test 4: JSON-like union (recursive type alias)
type JSONValue = str | int | float | bool | None | list[JSONValue] | dict[str, JSONValue]
eval_typing(JSONValue)
# Works! Returns: str | int | float | bool | None | list[JSONValue] | dict[str, JSONValue] ✅

# Test 5: Partial at top level (non-recursive)
class Address:
    street: str
    city: str
...
type PartialPerson = NewProtocol[
    *[
        Member[p.name, p.type | None, p.quals]
        for p in Iter[Attrs[Person]]
    ]
]
eval_typing(PartialPerson)  # Works! ✅
```

**Key Findings:**

1. **Classes work perfectly for recursion** - Use classes with type parameters
2. **Type aliases with self-reference work** - `JSONValue` recursive union works
3. **DeepPartial still impossible** - Cannot do automatic recursive optional
4. **Partial at top level works** - Using `Iter[Attrs[T]]` pattern
