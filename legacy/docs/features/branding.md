...
All proposed branding patterns were tested:

```python
from typing import Protocol
from typemap.type_eval import eval_typing
from typemap_extensions import NewProtocol, Member

# Test 1: Distinct Protocol classes
class UserIdDef:
    value: str
...
eval_typing(UserIdDef)  # Different class ✅
eval_typing(OrderIdDef)  # Different class ✅

# Test 2: Runtime brand creation
def create_brand(T, name):
    return NewProtocol[Member['value', T]]

UserId = create_brand(str, 'UserId')
OrderId = create_brand(str, 'OrderId')
# Works at runtime! ✅

# Test 3: Protocol-based branding
class Brand(Protocol):
    value: str

class UserIdProtocol(Brand):
    pass

class OrderIdProtocol(Brand):
    pass

# They satisfy Brand but are distinct classes
eval_typing(UserIdProtocol)  # ✅
eval_typing(OrderIdProtocol)  # ✅
UserIdProtocol is OrderIdProtocol  # False ✅

# Test 4: Class wrappers with validation
class Email:
    def __init__(self, v: str):
        if '@' not in v:
            raise ValueError('Invalid email')
...
email = Email('user@example.com')  # Works! ✅
password = Password('12345678')    # Works! ✅
```

**Key Findings:**

1. **NewProtocol works** - Creates distinct nominal types
2. **Runtime brand creation works** - Using `NewProtocol[Member['value', T]]`
3. **Protocol-based approach works** - Classes extending Protocol are distinct
4. **Class wrappers with validation work** - Runtime validation + type safety
...
*Last updated: 2026-03-03 (All patterns tested and work!)*
