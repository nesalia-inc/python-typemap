...
from typemap_extensions import Attrs, Iter, NewProtocol, Member, GetMemberType, KeyOf, Members
...
eval_typing(MemberTypes[User])  # → Protocol with name, email, age ✅
...
type NestedGet2[T, K1, K2] = GetMemberType[GetMemberType[T, K1], K2]
type City = NestedGet2[Person, Literal['address'], Literal['city']]
eval_typing(City)  # → <class 'str'> ✅

# Test 3: KeyOf
eval_typing(KeyOf[User])  # → (Literal['name'], Literal['email'], Literal['age']) ✅

# Test 4: GetMemberType
eval_typing(GetMemberType[User, Literal['name']])  # → <class 'str'> ✅

# Test 5: Members vs Attrs
class Service:
    def do_something(self) -> str: ...
    name: str

eval_typing(Attrs[Service])  # Only attributes ✅
eval_typing(Members[Service])  # Attributes + methods ✅
```

**Conclusion:** All member access helpers work perfectly!
