...
from typemap_extensions import GetArg
from typing import Literal, Callable

# Test 1: ReturnType
type MyReturnType[T] = GetArg[T, Callable, Literal[1]]
type R = MyReturnType[Callable[[int, str], bool]]
eval_typing(R)  # → <class 'bool'> ✅

# Test 2: ParamTypes
type MyParamTypes[T] = GetArg[T, Callable, Literal[0]]
type Params = MyParamTypes[Callable[[int, str], bool]]
eval_typing(Params)  # → tuple[Param[None, int], Param[None, str]] ✅

# Test 3: FirstParam
type FirstParam[T] = GetArg[GetArg[T, Callable, Literal[0]], tuple, Literal[0]]
type First = FirstParam[Callable[[int, str], bool]]
eval_typing(First)  # → Param[None, int] ✅

# Test 4: ArrayElem
type ArrayElem[T] = GetArg[T, list, Literal[0]]
type Elem = ArrayElem[list[str]]
eval_typing(Elem)  # → <class 'str'> ✅

# Test 5: DictValue
type DictValue[T] = GetArg[T, dict, Literal[1]]
type V = DictValue[dict[str, int]]
eval_typing(V)  # → <class 'int'> ✅

# Test 6: DictKey
type DictKey[T] = GetArg[T, dict, Literal[0]]
type K = DictKey[dict[str, int]]
eval_typing(K)  # → <class 'str'> ✅
```

**Conclusion:** All type extraction helpers work perfectly with existing operators!
